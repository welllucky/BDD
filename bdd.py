...
class BDD:
    ...
    def __init__(self, username, password, target_user, browser= 'chrome', test=False):
        self.__target = target_user
        self.__password = password
        self.__username = username
        self.__test = test
        self.__browser = browser.lower()

        if self.__browser == 'firefox':
            print('O Firefox esta temporariamente sem suporte, por favor utilize algum dos seguintes navegadores: chrome, opera ou edge. ')
            sleep(5)
            quit()
    ...
    def fall_process(self, rounds=None, duration = None):
        ...
        try:
            while True: ### Criar um jeito de verificar quando o twitter mandar yma tela de login no meio do processo, crie outra função para isso, só pegar o código do login
                ...
                if browser == 'chrome' or 'edge' or 'opera':
                    driver.find_element_by_css_selector('div.r-1loqt21:nth-child(9)').click()  #Ispecionar o botão de denuncia
    ...
    def user_finder(self):
        try:
            target = self.__target
            driver.get(f'https://www.twitter.com/{target}')
        except:
            assert ConnectionRefusedError('Página não encontrada')
            return 404
